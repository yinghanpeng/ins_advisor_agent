...
| `migrations/` | PostgreSQL、pgvector、业务记忆、RLS 和审批移除迁移 |
...
| `memory.yaml` | Session/Task/Preference TTL、容量、加密和保留策略 |
...
| `graph/builder.py` | 唯一代码执行顺序；通用路径与 Insurance Handler 分支 |
| `graph/nodes.py` | Guardrail、Memory、意图、工具、KYC、知识、生成和输出节点 |
...
| `skill.yaml` | 保险细分意图到代码 Handler 的声明性映射 |
| `prompts/strategy_generator.md` | 生成内容边界，不能拥有路由和状态写权限 |

旧 `skills/insurance_advisor/workflow.py` 已删除。保险 KYC、新闻、双知识库和策略逻辑位于 Python 中。

## Memory

| 模块 | 职责 |
| --- | --- |
| `memory/manager.py` | Session、Task、Preference 统一接口和内存实现 |
| `memory/redis_store.py` | Redis CAS、TTL、消息窗口、租户容量 |
| `memory/production_manager.py` | Redis 在线状态 + PostgreSQL 偏好/审计组合 |
...
Redis active intent 只保存控制信封，客户 KYC 值进入业务记忆。
