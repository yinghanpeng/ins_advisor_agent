...
- 短期/任务记忆：`session`、`task`，用于多轮指代、最近消息、active intent 和显式图状态恢复，可以每轮读取；
...
生产 DDL 以 `migrations/001_initial_pgvector.sql` 至 `migrations/006_simplify_short_term_memory.sql` 为准。
...
| Task | `agent:{tenant}:{session}:task` | 1 天 | current_state、final_answer_ready | WATCH/MULTI/EXEC + version CAS |

Redis Session/Task Hash 保存 `payload`、`version`、`updated_at`，Session 消息单独保存为有界 List，租户访问顺序保存为 ZSet。写入还会执行消息窗口裁剪、单 Payload 字节上限、
...
PostgreSQL 不参与在线 Session/Task 读取：`short_term_messages` 只保存加密原文、脱敏文本和 Hash，
用于审计与隐私导出。`task_memory` 已由迁移 006 删除，Task 快照只保存在带独立 TTL/CAS 的 Redis Key。
...
偏好抽取只保存短结构化值，例如 `response_style=简洁一点`、`response_language=中文`。写入时同步生成
Embedding 并写入 `memory_item_embeddings`，召回时按需执行 pgvector + 全文粗排与 Reranker 精排。完整用户原句、
...
`short_term_messages`、`conversation_messages`、分析输入/证据和生成输出原文使用 pgcrypto AES-256
...
删除客户时按 Lineage、KYC、Event、Output、Analysis、Session、Message、Conversation、Case、Fact、Consent
的顺序清理；Embedding 通过外键级联。导出默认不解密原始消息。`make memory-retention` 根据
`expires_at` 分租户、分表、分批清理，不执行全表大事务。

### Migration 约束

`scripts/db_upgrade.py` 使用 advisory lock、`schema_migrations` 台账、SHA-256 校验和和单 migration
事务。`004_legacy_memory_upgrade.sql` 会把旧 `long_term_memory_items` 拆成事实表和向量表，并用部署时
提供的密钥加密旧短期消息/生成输出后删除明文列。已执行 migration 不能原地修改，结构变化必须新增
下一编号文件。
