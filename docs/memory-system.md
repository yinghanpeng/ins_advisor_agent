...
生产 DDL 以 `migrations/001_initial_pgvector.sql` 至 `migrations/005_remove_runtime_approval.sql` 为准。
...
PostgreSQL 不参与在线 Session 窗口读取，只保留恢复/审计数据：`short_term_messages` 保存加密原文、
脱敏文本和 Hash；`task_memory` 保存带 `source_version` 的任务快照，较旧 Redis 版本不能倒灌覆盖新快照。
