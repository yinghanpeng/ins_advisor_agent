...
- **通用 Agent Core**：状态机、工具、Guardrail、RAG、Memory、成本、恢复和可观测性边界；
- **Insurance Advisor Domain Skill**：保险破冰、客户异议处理、KYC 教练和低压沟通策略；
- **Sales Intelligence Layer**：把一线销售访谈加工为结构化、可审核、可检索的销售洞察卡片。
- **Agent Capability & Prompt Registry**：统一管理独立版本化的 Tool、Skill、Prompt，提供审核发布、
  环境绑定、稳定快照、按需加载、依赖影响和审计能力。
...
项目希望解决的不只是“模型能否回答”，而是一次 Agent 请求能否被约束、解释、测试和恢复：
...
| Session/Task/Preference Memory | 已实现 | FastAPI 使用 Redis Session/Task + PostgreSQL Preference；CLI/测试保留内存实现 |
...
| Tool/Skill/Prompt Registry | 核心控制面已实现 | 严格 Manifest、SemVer、不可变发布、RLS 持久化、Alias/Binding、Eval Gate、审批、审计和管理 API |
| 多意图 DAG 与安全并行 | 已实现 | 依赖/Fact/读写冲突确定性计算；只并发安全读取；写入串行并在高风险动作前暂停确认 |
| RuntimeSnapshot 与按需加载 | 已实现 | Run 开始固定精确版本/Hash；Catalog 不含完整 Schema/Skill/Prompt；选中和调用时再加载 |
| Dify | Control Plane 配置已提供 | 通过 HTTP 接 Agent Core，生产认证和部署需继续完成 |
| 多 Agent/Handoff | 路线预留 | 当前仍是单 Agent 显式状态机，不是多 Agent 框架 |
...
- Redis client：承载 Session/Task、CAS、TTL、租户容量限制和网关限流
...
flowchart TD
    ...
    Intent --> Skills["Domain Skill Router"]
    Graph --> Memory["Session / Task / Preference / Business Memory"]
    Graph --> Rag["RAG and Context Engineering"]

    General --> Tools["Tool Registry and Executor"]
    Skills --> Insurance["Code-native Insurance Conversation Handler"]
    Insurance --> Sales["Sales Intelligence Layer"]
    Insurance --> DualKB["Methods KB + Contract/Compliance KB"]
    Memory --> Redis["Redis Active Intent / Session / Task"]

    Graph --> Trace["Local Structured Trace"]
    Graph -. optional .-> LangSmith["LangSmith Adapter"]
    Memory -. production adapter .-> Postgres["PostgreSQL / pgvector"]
    Rag -. production adapter .-> Postgres
...
- **Agent Core Data Plane**：负责状态、工具执行、租户字段、Guardrail、Trace、Memory、RAG、成本和恢复。
...
- `src/agent_core/graph/builder.py`：统一代码链路与 Insurance Handler；
...
- `src/agent_core/registry/`：Artifact 领域模型、生命周期、PostgreSQL、上传校验、Resolver 和 Prompt Renderer；
- `src/agent_core/orchestration/`：Task Schema、依赖/冲突、DAG 批次和有界执行器；
- `docs/agent-platform-architecture.md`、`docs/agent-runtime-flow.md` 和
  `docs/tool-skill-prompt-metadata-reference.md`：Registry 架构与公开契约。

启用 FastAPI 后，管理台位于 `/registry`，管理 API 位于 `/api/registry`。请求必须同时通过
现有租户网关认证和 `X-Registry-Admin-Token`，角色由 `X-Actor-Roles` 传入。示例 Artifact 包在
`examples/artifacts/`，常用离线命令如下：

```bash
PYTHONPATH=src python3 -m agent_core.registry.cli schema prompt
PYTHONPATH=src python3 -m agent_core.registry.cli validate /path/to/artifact-package.zip
...
| Task Memory | 当前任务状态、是否已生成最终答案 | FastAPI: Redis + PostgreSQL 恢复快照 |
...
- Redis Session 元数据/Task 使用 Hash，最近消息使用 `agent:{tenant}:{session}:messages` List，容量访问顺序使用 ZSet；整体受 WATCH/MULTI/EXEC、版本号、TTL、Payload 上限和租户级 LRU 约束；
...
- 原始消息、分析输入、匹配证据与生成输出正文使用 pgcrypto 加密，低权限查询只读取脱敏副本和 Hash；
...
DeepEval 作为可选补充评分层接在同一套 Eval Harness 最后：

```bash
uv sync --extra dev --extra eval
python3 evals/run_evals.py --suite business_quality --max-trials 1 --enable-deepeval
# 或：make eval-deepeval
```

该命令仍通过 `AgentRunRequest → WorkflowEngine.run → AgentRunResponse` 执行正式链路，
自研规则与可选原生 Judge 会先完整执行；随后只把声明了 `judge` 的最终回答和正式链路
已选出的脱敏检索上下文交给本地 DeepEval G-Eval。DeepEval 不重新检索、不改写状态、
也不替换已有断言。默认不登录或上传 Confident AI，
DeepEval 自动 dotenv 与匿名遥测也会关闭；阈值由 `configs/eval_gates.yaml` 管理。
...
本地 Eval 数据集位于 `evals/dataset.jsonl`，按 Suite 维护版本化 JSONL Case。每个 Case/Trial
使用独立 `WorkflowEngine`，只有同一多轮 Case 内复用 Session，防止记忆串扰。Runner 会检查答案、Schema、
意图/领域路由、工具、护栏、状态轨迹、Trace 和成本，并输出 `evals/reports/latest.json` 与 JUnit 报告。
字段说明、新增 Case 方法和发布门禁建议见 [Evaluation 评估体系](docs/evaluation.md)。
...
| `domain_skills.yaml` | Domain Skill 注册与启用状态 | 否，当前路由仍由代码实现 |
...
| `memory.yaml` | 召回策略、Session/Task TTL、Redis 容量/CAS、加密密钥名和 Retention | 是 |
...
迁移按编号位于 `migrations/`：`001` 创建通用 Agent/Memory/RAG 表，`002` 创建 KYC 业务记忆表，
`003` 强制 RLS，`004` 负责旧短期消息加密和旧 `long_term_memory_items` 数据迁移，`005` 删除历史
运行时人工审批表和索引。`db_upgrade.py`
...
1. 在 `src/agent_core/skills/<skill_name>/` 创建 Skill；
2. 定义 `skill.yaml`、代码处理器、Prompt 和输出契约；
3. 在 `intent_catalog.yaml` 与 Domain Router 中注册；
4. 定义所需 Memory、RAG、Tool 和 Guardrail；
5. 在 `AgentState` 中增加必要的显式字段，避免把关键状态藏进 metadata；
6. 补充 Step Contract、测试和 Eval Case；
7. 同步 `configs/domain_skills.yaml`。

当前只实现 `insurance_advisor`。Research、Document Analysis、Interview Assistant、Customer Service 和
Data Analysis 仍是关闭的路线预留。
...
- Session/Task/Preference Memory 和业务记忆模型；
- 本地 Hybrid RAG 与销售洞察检索；
- PostgreSQL/pgvector Repository 与生产 RAG 类；
- Redis Session/Task、PostgreSQL Preference 和业务记忆生产 Runtime；
